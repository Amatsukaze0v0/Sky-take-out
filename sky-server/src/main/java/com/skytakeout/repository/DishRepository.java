...
public interface DishRepository extends JpaRepository<Dish, Long> {
    ...
    @Query("select d from Dish d JOIN Category c ON d.categoryId = c.id where d.categoryId = :categoryId")
    List<Dish> findByCategroyId(Long categoryId);
    ...
    @Query("select count(d.id) from Dish d where d.status = :status")
